...
export default function Sidebar({
  ...
  return (
    <>
      ...
      <motion.aside
        id="sidebar"
        className={`fixed inset-y-0 left-0 z-[100] w-64 bg-white dark:bg-[#1F1F23] border-r border-gray-200 dark:border-[#333] lg:translate-x-0 lg:static overflow-y-auto max-h-screen`}
        initial={{ x: -300 }} // Initial state, hidden off-screen
        animate={{ x: isSidebarOpen ? 0 : -300 }} // Animate to 0 when open, -300 when closed
        exit={{ x: -300 }} // Exit off-screen when closed
        transition={{ type: "spring", stiffness: 300, damping: 30 }} // Add spring easing for smooth transitions
      >
              ...
              {section.items && section.items.length > 0 ? (
                <div className="p-2 space-y-4">
                  <motion.div
                    ...
                    whileHover={{ scale: 1.05 }} // Add hover scale effect with Framer Motion
                  >
                  ...
                  <motion.div
                    className={`${openDropdown === index ? "block" : "hidden"} ms-4`}
                    initial={{ opacity: 0 }}
                    animate={{ opacity: openDropdown === index ? 1 : 0 }}
                    transition={{ duration: 0.3 }}
                  >
